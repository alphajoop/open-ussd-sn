...
const UssdTable: React.FC<UssdTableProps> = ({ data, currentPage, itemsPerPage }) => {
      ...
      <Table>
        ...
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-[100px] hidden md:table-cell">{t("common.status")}</TableHead>
            <TableHead className="w-[150px] hidden md:table-cell">{t("common.lastUpdate")}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
            ...
            paginatedData.map((item, index) => (
              <TableRow 
                ...
                className={cn(
                  ...
                  selectedRow === `${item.operateur}-${item.codeUssd}` && "highlight-row"
                )}
                onClick={() => setSelectedRow(`${item.operateur}-${item.codeUssd}`)}
              >
                <TableCell>
                ...
                <TableCell className="font-mono font-bold">{item.codeUssd}</TableCell>
                <TableCell className="hidden md:table-cell">{item.description}</TableCell>
                <TableCell className="hidden md:table-cell">
                  <Badge variant={item.statut === "Actif" ? "default" : "destructive"}>
                    {item.statut}
                  </Badge>
                </TableCell>
                <TableCell className="hidden md:table-cell">{item.derniereMiseAJour}</TableCell>
              </TableRow>
            ))
          )}
        </TableBody>
